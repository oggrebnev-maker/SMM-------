...
try {
    ...
    match(true) {
        ...
        $method === "GET"    && preg_match('#^/projects/(\d+)/watermark/image$#', $uri, $m) => (new ProjectController())->getWatermarkImage((int)$m[1]),
        ...
        $method === "POST"   && preg_match('#^/projects/(\d+)/watermark$#', $uri, $m) => (new ProjectController())->uploadWatermark((int)$m[1]),
        $method === "PUT"    && preg_match('#^/projects/(\d+)/watermark$#', $uri, $m) => (new ProjectController())->updateWatermark((int)$m[1]),
        $method === "DELETE" && preg_match('#^/projects/(\d+)/watermark$#', $uri, $m) => (new ProjectController())->deleteWatermark((int)$m[1]),
