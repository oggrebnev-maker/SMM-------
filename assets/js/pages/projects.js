...
const PageProjects = (() => {
  ...
  function renderProjectCard(p, current) {
    ...
    return `
      <div draggable="true" data-id="${p.id}"
        style="display:flex;flex-wrap:wrap;align-items:center;gap:14px;padding:14px 16px;background:var(--surface);border:1.5px solid ${isActive ? 'var(--accent)' : 'var(--border)'};border-radius:12px;cursor:default;transition:box-shadow .15s,border-color .15s;${isActive ? 'background:var(--accent-lt);' : ''}">
  ...
  function bindCardEvents() {
    ...
    document.querySelectorAll('.proj-settings-btn').forEach(btn => {
      btn.addEventListener('click', () => openSettingsPanel(btn.dataset.id));
    });
  ...
  function openAddChannelPanel(pid) {
